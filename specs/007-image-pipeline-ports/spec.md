...
- Q: How does the storage adapter pick its content type? → A: The pipeline does NOT pass content type through the port — the production R2 adapter hard-codes the same value as the legacy `uploadToR2` (`image/webp`, matching the existing `${logId}/cover.webp` key). DALL-E 3 / `gpt-image-1.x` always returns PNG bytes, but the pre-existing object key/content-type mismatch is preserved verbatim to avoid touching production behaviour. Format detection or correction is out of scope.
...
- **FR-005b**: The image storage port's `put(logId, bytes)` signature MUST NOT carry a `contentType` argument. The production R2 adapter sets the content type internally to match the existing `uploadToR2` behaviour. Image-format detection is out of scope.
