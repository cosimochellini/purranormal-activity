...
import { createStoryForge } from './index'
import type { CategoriesPort, LlmPort } from './types'
...
interface RecordingLlm extends LlmPort {
  calls: { model: string; prompt: string }[]
}

const recordingLlm = (responses: string[] = []): RecordingLlm => {
...
interface FakeCategoriesPort extends CategoriesPort {
  fetched: number
}

const fakeCategoriesPort = (rows: Category[] = sampleCategories): FakeCategoriesPort => {
  const port: FakeCategoriesPort = {
    fetched: 0,
    async all() {
      ...
      port.fetched++
      const fetched = rows
      if (myToken === token) cache = fetched
      return fetched
    },
    invalidate() {
      cache = null
...
const fakeStyle = () => ImageStyle.ANIME

const sampleLog: LogWithCategories = {
  id: 21,
...
describe('storyForge.questions', () => {
  it('returns ok:true with parsed array on valid response', async () => {
    const llm = recordingLlm([
      JSON.stringify([{ question: 'Where?', availableAnswers: ['Home', 'Park'] }]),
    ])
    const sf = createStoryForge({ categories: fakeCategoriesPort(), llm, randomStyle: fakeStyle })
    const r = await sf.questions('event')
    expect(r).toEqual({
      ok: true,
      value: [{ question: 'Where?', availableAnswers: ['Home', 'Park'] }],
    })
    expect(llm.calls[0]?.model).toBe('gpt-5-mini')
  })

  it('returns ok:false error:parse on invalid JSON', async () => {
    const llm = recordingLlm(['not-json'])
    const sf = createStoryForge({ categories: fakeCategoriesPort(), llm, randomStyle: fakeStyle })
    const r = await sf.questions('event')
  ...
  it('returns ok:false error:validation on shape mismatch', async () => {
    const llm = recordingLlm([JSON.stringify([{ question: 'Where?', availableAnswers: 'Home' }])])
    const sf = createStoryForge({ categories: fakeCategoriesPort(), llm, randomStyle: fakeStyle })
    const r = await sf.questions('event')
    expect(r.ok).toBe(false)
  ...
  it('returns ok:false error:model when llm throws Error', async () => {
    const llm: LlmPort = {
      async text() {
    ...
    const sf = createStoryForge({ categories: fakeCategoriesPort(), llm, randomStyle: fakeStyle })
    const r = await sf.questions('event')
    expect(r).toEqual({ ok: false, error: 'model', message: 'rate limit' })
  })

  it('handles non-Error throws (string) with a printable message', async () => {
    const llm: LlmPort = {
      async text() {
        // biome-ignore lint/suspicious/noExplicitAny: testing non-Error throws
        throw 'oops' as any
      },
    }
    const r = await createStoryForge({
      categories: fakeCategoriesPort(),
      llm,
      randomStyle: fakeStyle,
    }).questions('e')
    expect(r).toEqual({ ok: false, error: 'model', message: 'oops' })
  })

  it('handles non-Error throws (undefined) with a printable message', async () => {
    const llm: LlmPort = {
      async text() {
        // biome-ignore lint/suspicious/noExplicitAny: testing non-Error throws
  ...
  it('treats empty llm response as JSON "[]" → ok:true with empty array', async () => {
    const llm = recordingLlm([''])
    const sf = createStoryForge({ categories: fakeCategoriesPort(), llm, randomStyle: fakeStyle })
    const r = await sf.questions('event')
...
describe('storyForge.logDetails', () => {
  it('returns ok:true on a valid response', async () => {
    ...
    const sf = createStoryForge({ categories: fakeCategoriesPort(), llm, randomStyle: fakeStyle })
    const r = await sf.logDetails('event', [{ question: 'Q', answer: 'A' }])
    expect(r.ok).toBe(true)
    if (r.ok) {
      expect(r.value.title).toBe('T')
      expect(r.value.categories).toEqual([{ id: 1, name: 'magic' }])
    }
    expect(llm.calls[0]?.model).toBe('gpt-5.2')
  })
  ...
  it('returns ok:false error:validation on missing fields', async () => {
    const llm = recordingLlm([JSON.stringify({ title: '', description: 1, categories: [] })])
    const sf = createStoryForge({ categories: fakeCategoriesPort(), llm, randomStyle: fakeStyle })
    const r = await sf.logDetails('event', [])
    expect(r.ok).toBe(false)
    if (!r.ok) expect(r.error).toBe('validation')
  ...
  it('returns ok:false error:model when categories.all rejects', async () => {
    ...
    const r = await sf.logDetails('event', [])
    expect(r).toEqual({ ok: false, error: 'model', message: 'db down' })
  })

  it('returns ok:false error:model when llm throws', async () => {
    const sf = createStoryForge({
      categories: fakeCategoriesPort(),
      llm: {
        async text() {
    ...
    const r = await sf.logDetails('event', [])
    expect(r).toEqual({ ok: false, error: 'model', message: 'OpenAI exploded' })
  })
})

// ---------- imagePrompt ----------

describe('storyForge.imagePrompt', () => {
  it('returns ok:true with the raw prompt text', async () => {
    const llm = recordingLlm(['the final image prompt'])
    const sf = createStoryForge({ categories: fakeCategoriesPort(), llm, randomStyle: fakeStyle })
    const r = await sf.imagePrompt('event')
    expect(r).toEqual({ ok: true, value: 'the final image prompt' })
    expect(llm.calls[0]?.model).toBe('gpt-5.2')
  })

  it('returns ok:false error:validation on empty response', async () => {
    const llm = recordingLlm([''])
    const sf = createStoryForge({ categories: fakeCategoriesPort(), llm, randomStyle: fakeStyle })
    const r = await sf.imagePrompt('event')
    expect(r.ok).toBe(false)
  ...
  it('returns ok:false error:model when llm throws', async () => {
    ...
    const r = await sf.imagePrompt('event')
    expect(r).toEqual({ ok: false, error: 'model', message: 'timeout' })
  })
})

// ---------- telegramMessage ----------

describe('storyForge.telegramMessage', () => {
  it('returns ok:true with stripped HTML fences', async () => {
    const llm = recordingLlm(['```html\n<b>Hello</b>\n```'])
    const sf = createStoryForge({ categories: fakeCategoriesPort(), llm, randomStyle: fakeStyle })
    const r = await sf.telegramMessage(sampleLog)
    expect(r).toEqual({ ok: true, value: '<b>Hello</b>' })
    expect(llm.calls[0]?.model).toBe('gpt-5-mini')
  })

  it('returns ok:false error:validation when message becomes empty after fence-strip', async () => {
    const llm = recordingLlm([''])
    const sf = createStoryForge({ categories: fakeCategoriesPort(), llm, randomStyle: fakeStyle })
    const r = await sf.telegramMessage(sampleLog)
    expect(r.ok).toBe(false)
  ...
  it('returns ok:false error:model when llm throws', async () => {
    const sf = createStoryForge({
      ...
      llm: {
        async text() {
          throw new Error('telegram model down')
        },
      },
      randomStyle: fakeStyle,
    })
...
// ---------- categories cache lifecycle ----------

describe('categories cache lifecycle (via storyForge)', () => {
  it('calls categories.all() with cache reuse across multiple logDetails invocations', async () => {
    const cats = fakeCategoriesPort()
    const llm = recordingLlm([
      JSON.stringify({ title: 'a', description: 'b', categories: [], imageDescription: 'c' }),
      JSON.stringify({ title: 'd', description: 'e', categories: [], imageDescription: 'f' }),
    ])
    const sf = createStoryForge({ categories: cats, llm, randomStyle: fakeStyle })
    ...
    // The fake's cache returns the cached rows on the second call without
    // incrementing `fetched` — exactly the behaviour we want from the real
    // production adapter.
    expect(cats.fetched).toBe(1)
  })

  it('re-fetches after invalidateCategories()', async () => {
    const cats = fakeCategoriesPort()
    const llm = recordingLlm([
      JSON.stringify({ title: 'a', description: 'b', categories: [], imageDescription: 'c' }),
      JSON.stringify({ title: 'd', description: 'e', categories: [], imageDescription: 'f' }),
    ...
    sf.invalidateCategories()
    await sf.logDetails('event', [])
    expect(cats.fetched).toBe(2)
  })
})
...
describe('prompt snapshots', () => {
  it('CREATE_QUESTIONS_PROMPT', async () => {
    const llm = recordingLlm(['[]'])
    const sf = createStoryForge({ categories: fakeCategoriesPort(), llm, randomStyle: fakeStyle })
    await sf.questions('Spooky kitten event')
    expect(llm.calls[0]?.prompt).toMatchInlineSnapshot(`
      "
  ...
  it('GENERATE_LOG_DETAILS_PROMPT', async () => {
    const llm = recordingLlm([
      JSON.stringify({
        title: 't',
        description: 'd',
    ...
    const sf = createStoryForge({
      ...
      llm,
      randomStyle: () => ImageStyle.ANIME,
    })
    await sf.logDetails('A purple kitten teleported the chick', [
      { question: 'Where?', answer: 'Kitchen' },
    ])
    expect(llm.calls[0]?.prompt).toMatchInlineSnapshot(`
      "
        ...
        La descrizione del pulcino dell'attività paranormale è:
        "A purple kitten teleported the chick"
  ...
  it('GENERATE_IMAGE_PROMPT', async () => {
    const llm = recordingLlm(['out'])
    const sf = createStoryForge({
      categories: fakeCategoriesPort(),
      llm,
      randomStyle: () => ImageStyle.GIBHILI,
    })
    await sf.imagePrompt('cat conjures rain')
    expect(llm.calls[0]?.prompt).toMatchInlineSnapshot(`
      "
  ...
  it('GENERATE_TELEGRAM_PROMPT', async () => {
    const llm = recordingLlm(['<b>Hi</b>'])
    const sf = createStoryForge({ categories: fakeCategoriesPort(), llm, randomStyle: fakeStyle })
    await sf.telegramMessage(sampleLog)
    expect(llm.calls[0]?.prompt).toMatchInlineSnapshot(`
      "
        ...
        JSON: {"id":21,"title":"Strange noises","description":"desc","createdAt":1,"updatedAt":2,"status":"ImageGenerated","error":null,"imageDescription":"imgdesc","categories":[1,2]}
      ...
      <tg-emoji emoji-id="5368324170671202286">👍</tg-emoji>
        ...
        - Il messaggio deve sempre iniziare con #DAY 42
        ...
        - Il link deve essere: https://purranormal-activity.pages.dev/21
